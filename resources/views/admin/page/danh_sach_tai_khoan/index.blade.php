...
var vueApp = new Vue({
    ...
    methods: {
        loadData() {
            axios.post('{{ Route('taiKhoanData') }}').then(r => { this.list_tai_khoan = r.data.xxx || []; });
        },
        doiTrangThaiBlock(v) {
            axios.post('{{ Route('taiKhoanBlock') }}', v).then(r => {
                if (r.data.status) { toastr.success(r.data.message); this.loadData(); }
                else toastr.error(r.data.message);
            });
        },
        doiTrangThai(v) {
            axios.post('{{ Route('taiKhoanStatus') }}', v).then(r => {
                if (r.data.status) { toastr.success(r.data.message); this.loadData(); }
                else toastr.error(r.data.message);
            });
        },
...
function themTK() {
    ...
    axios.post('{{ Route('taiKhoanStore') }}', data).then(r => {
...
function capNhatTK() {
    ...
    axios.post('{{ Route('taiKhoanUpdate') }}', data).then(r => {
...
function xoaTK() {
    axios.post('{{ Route('taiKhoanDel') }}', { id: document.getElementById('del-id').value }).then(r => {
