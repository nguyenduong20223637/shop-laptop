...
var vueApp = new Vue({
    ...
    methods: {
        taoDM() {
            ...
            axios.post("{{ Route('DanhMucCreate') }}", this.danh_muc)
                .then((res) => {
        ...
        doiStatus(payload) {
            axios.post('{{ Route('DanhMucStatus') }}', payload)
                .then((res) => {
...
function capNhatDM() {
    ...
    axios.post('{{ Route('DanhMucUpdate') }}', data)
        .then((res) => {
...
function xoaDM() {
    axios.post('{{ Route('DanhMucDestroy') }}', { id: document.getElementById('del-id').value })
        .then((res) => {
            if (res.data.status == 1) {
                toastr.success(res.data.message);
