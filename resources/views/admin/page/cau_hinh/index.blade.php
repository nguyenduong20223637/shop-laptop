...
var vueApp = new Vue({
    ...
    methods: {
        taoCauHinh() {
            ...
            axios.post("{{ Route('CauHinhCreate') }}", this.cau_hinh).then(r => {
        ...
        doiStatus(v) {
            axios.post('{{ Route('CauHinhStatus') }}', v).then(r => {
        ...
        loadData() {
            axios.post('{{ Route('CauHinhData') }}').then(r => { this.List_Opt = r.data.data; });
        }
    },
});
...
function capNhatCH() {
    ...
    axios.post('{{ Route('CauHinhUpdate') }}', {
...
function xoaCH() {
    axios.post('{{ Route('CauHinhDestroy') }}', { id: document.getElementById('del-id').value }).then(r => {
