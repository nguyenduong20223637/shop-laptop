...
{{-- Thanh thông báo top đã có ở header, ẩn đi --}}
  ...
  <div class="container" style="padding-top:4px;">
    ...
    <div class="hp-hero3">
      ...
      <div class="hp-cat">
        ...
        <ul class="hp-cat__list">
          ...
          @endphp
          @foreach(\App\Models\ThuongHieu::where('trang_thai',1)->get() as $th)
          <li><a href="/home/thuong-hieu/{{ $th->id }}" class="hp-cat__brand">
            @if(isset($brandLogosMenu[$th->ten_thuong_hieu]))
              <img src="{{ $brandLogosMenu[$th->ten_thuong_hieu] }}" alt="{{ $th->ten_thuong_hieu }}">
            @else
              <i class="fa-solid fa-tag"></i>
            @endif
            <span>{{ $th->ten_thuong_hieu }}</span>
          </a></li>
          @endforeach
        </ul>
      </div>
      ...
      <div class="hp-hero">
        <div class="hp-slides" id="hpSlides">
          ...
          <a href="{{ $banner->duong_dan }}" class="hp-slide" style="background-image:url('{{ $banner->hinh_anh }}')"></a>
          @endforeach
          @if($banners->isEmpty())
          <a href="/home/all-product" class="hp-slide" style="background-image:url('{{ asset('assets_client/imagesbanner/banner1.jpg') }}')"></a>
          @endif
        </div>
        <div class="hp-dots" id="hpDots">
          @foreach($banners as $k => $banner)
          <div class="hp-dot {{ $k === 0 ? 'active' : '' }}" onclick="hpGoTo({{ $k }})"></div>
          @endforeach
          @if($banners->isEmpty())
    ...
    <div class="hp-brands-row">
        ...
        <div class="hp-brands-row__list">
            @foreach(\App\Models\ThuongHieu::where('trang_thai',1)->get() as $th)
            <a href="/home/thuong-hieu/{{ $th->id }}" class="hp-brand-card">
                @if(isset($brandLogos[$th->ten_thuong_hieu]))
                    <img src="{{ $brandLogos[$th->ten_thuong_hieu] }}" alt="{{ $th->ten_thuong_hieu }}" class="hp-brand-card__logo">
                @endif
                <span class="hp-brand-card__name">{{ $th->ten_thuong_hieu }}</span>
            </a>
            @endforeach
        </div>
    </div>
...
<div class="hp" id="home-app" v-cloak>
  ...
  <div class="container">
    ...
    <div class="deal-banner-wrap" onclick="window.location='/home/all-product'">
      <img src="{{ asset('assets_client/imagesbanner/banner2.jpg') }}" class="deal-banner-img" alt="Deal laptop">
    ...
    <div class="nc-section">
      ...
      <div class="nc-grid">
        @foreach($needItems as $n)
        <a href="{{ $n['url'] }}" class="nc-card" style="--nc-bg:{{ $n['bg'] }}">
          <div class="nc-card__top">
            <div class="nc-card__icon-wrap" style="background:{{ $n['grad'] }}">
              <i class="fa-solid {{ $n['icon'] }}"></i>
            </div>
            <span class="nc-card__label">{{ $n['label'] }}</span>
            <span class="nc-card__arrow"><i class="fa-solid fa-arrow-right"></i></span>
          </div>
          <div class="nc-card__img-wrap">
            @if($n['prods']->count())
              <img src="{{ $n['prods']->first()->hinh_anh }}" alt="{{ $n['label'] }}">
            @else
