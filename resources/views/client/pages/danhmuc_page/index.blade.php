...
<div class="fpt-page" id="app2" v-cloak>
  ...
  <div class="container">
    ...
    {{-- Banner --}}
    <div class="fpt-banner" style="background-image:url('{{ $danhmuc->hinh_anh }}')">
      <div class="fpt-banner__text">
        <h2>{{ $danhmuc->ten_danh_muc }}</h2>
    ...
    <div class="fpt-deal-banner">
      ...
      <div class="fpt-deal-banner__products">
        @foreach(array_slice($bestSales->all() ?? [], 0, 4) as $v)
        <a href="/home/product-detail/{{ $v->id }}" class="fpt-deal-item">
          <img src="{{ $v->hinh_anh }}" alt="{{ $v->ten_san_pham }}">
          <p class="fpt-deal-item__name">{{ Str::limit($v->ten_san_pham, 30) }}</p>
          <p class="fpt-deal-item__price">{{ number_format($v->gia, 0, ',', '.') }}₫</p>
        </a>
        @endforeach
      </div>
    </div>
    ...
    <div class="fpt-blog">
      <div class="fpt-blog__img">
        <img src="{{ $danhmuc->hinh_anh }}" alt="{{ $danhmuc->ten_danh_muc }}">
      </div>
...
new Vue({
  ...
  methods: {
    loadData(page=1) {
      axios.post('{{ Route("DanhMucDataProduct") }}', { id: $('#id_danh_muc').val() }, { page })
        .then(res => {
    ...
    SortProduct() {
      ...
      axios.post('{{ Route("sortProducts") }}', { select: this.SortProd, data: this.List_products })
