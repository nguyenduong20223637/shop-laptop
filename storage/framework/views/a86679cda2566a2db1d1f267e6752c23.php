...
var vueApp = new Vue({
    ...
    methods: {
        load: function() {
            var self = this;
            axios.post('<?php echo e(Route("BannerData")); ?>').then(function(r) {
        ...
        themMoi: function() {
            ...
            var self = this;
            axios.post('<?php echo e(Route("BannerCreate")); ?>', this.banner).then(function(r) {
                if (r.data.status) {
        ...
        doiStatus: function(v) {
            var self = this;
            axios.post('<?php echo e(Route("BannerStatus")); ?>', { id: v.id }).then(function(r) {
        ...
        doiThuTu: function(fromIdx, toIdx) {
            ...
            for (var i = 0; i < listCopy.length; i++) {
                updates.push(axios.post('<?php echo e(Route("BannerUpdate")); ?>', {
                    id: listCopy[i].id,
...
function capNhat() {
    ...
    axios.post('<?php echo e(Route("BannerUpdate")); ?>', data).then(function(r) {
        if (r.data.status) {
            toastr.success(r.data.message);
...
function xoa() {
    axios.post('<?php echo e(Route("BannerDestroy")); ?>', { id: document.getElementById('xoa-id').value }).then(function(r) {
        if (r.data.status) {
            toastr.success(r.data.message);
...
<?php echo $__env->make('admin.share.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\Shop-laptop\resources\views/admin/page/banner/index.blade.php ENDPATH**/ ?>