        ...
        <div class="site-header-right">
          ...
          <div class="nav-account-wrap" id="accountWrap">
            <a href="#" class="site-header-account-btn acc-logged-btn">
              <div class="acc-avatar-sm"><?php echo e(strtoupper(substr(Session::get('auth')->ho_va_ten, 0, 1))); ?></div>
              <span><?php echo e(Str::limit(Session::get('auth')->ho_va_ten, 12)); ?></span>
            </a>
            <div class="acc-dropdown" id="accountDropdown">
              <div class="acc-dropdown__header acc-dropdown__header--logged">
                <div class="acc-avatar-lg"><?php echo e(strtoupper(substr(Session::get('auth')->ho_va_ten, 0, 1))); ?></div>
                <div>
                  <div class="acc-dropdown__title"><?php echo e(Session::get('auth')->ho_va_ten); ?></div>
...
<script>
    ...
    new Vue({
        ...
        methods: {
            logout() {
                axios.post('<?php echo e(Route('clientLogout')); ?>').then((res) => {
                    if (res.data.status == 1) setTimeout(() => { window.location.href = '/'; }, 500);
                });
            },
            CountProduct() {
                axios.post('<?php echo e(Route('CountCart')); ?>').then((res) => {
                    $('#countofprod').text(res.data.status == 1 ? res.data.data : 0);
                }).catch(() => {});
            },
            loaddanhmuc() {
                axios.post('<?php echo e(Route('loadDanhMucMenu')); ?>').then((res) => {
                    if (res.data.status == 1) this.list_danhmuc = res.data.data;
                }).catch(() => {});
            },
            loadthuonghieu() {
                axios.post('<?php echo e(Route('loadThuongHieuMenu')); ?>').then((res) => {
...
<?php /**PATH C:\xampp\htdocs\Shop-laptop\resources\views/client/share/nav.blade.php ENDPATH**/ ?>